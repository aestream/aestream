...
struct EVT3 : FileBase
{
    ...
    std::tuple<std::vector<AER::Event>, size_t> read_events(const int64_t n_events = -1)
    {
        ...
        do
        {
            ...
            auto offset = decode_event_buffer(buffer, size, events, n_events < 0 ? buffer_size : n_events - events.size());
            fseek(fp.get(), offset * sizeof(uint16_t), SEEK_CUR); // Re-align file if we didn't process all events
...
private:
    ...
    inline ContainerProcessingState process_vector_event(std::vector<AER::Event> &events, int64_t remaining_events)
    {
        if (!y_event.has_value() || !x_event.has_value())
        {
            throw std::runtime_error("EVT3 file format error: no header data for base (x, y) coordinates given");
        }
        ...
        for (; i < state.bit_size; ++i)
        {
            if (state.bits & (1U << i))
            {
              const AER::Event event = {current_time, 
                static_cast<uint16_t>(x_event.value().coordinate), y, x_event.value().meta};
              events.push_back(std::move(event));
              remaining_events--;
            }
            x_event.value().coordinate++; // Increment coordinate irregardless of validity
    ...
    int32_t decode_event_buffer(uint16_t *buffer, size_t buffer_size, std::vector<AER::Event> &events, const size_t remaining_events)
    {
        ...
        return i - static_cast<int64_t>(buffer_size); // Return offset for unprocessed events
    }
