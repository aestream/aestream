...
DVS2UDP<T>::DVS2UDP(uint32_t interval, uint32_t bfsize, std::string port, std::string IP) {
  ...
  if ((rv = getaddrinfo(IPAdress.c_str(), serverport.c_str(), &hints, &servinfo)) != 0) {
...
void DVS2UDP<T>::sendpacket(Generator<T>& input_generator, bool include_timestamp) {
    ...
    for (AEDAT::PolarityEvent event : input_generator) {
        if (event.valid == true){
            ...
            if (include_timestamp) {
                message[current_event] = (event.x & 0x7FFF)  << 16; // Be aware that for machine-independance it should be: htons(polarity_event.x & 0x7FFF);
                message[current_event + 1] = event.timestamp;
            } else {
                message[current_event] = (event.x | 0x8000) << 16; // Be aware that for machine-independance it should be: htons(polarity_event.x | 0x8000);
            }

            if (event.polarity) {
                message[current_event] |= event.y | 0x8000; // Be aware that for machine-independance it should be: htons(polarity_event.y | 0x8000);
            } else {
            message[current_event] |= event.y & 0x7FFF; // Be aware that for machine-independance it should be: htons(polarity_event.y & 0x7FFF);
            }
    ...
    if (sent == false) {
        if ((numbytes = sendto(sockfd, &message, current_event * event_size, 0, p->ai_addr, p->ai_addrlen)) == -1) {
            perror("talker error: sendto");
            exit(1);
        }
